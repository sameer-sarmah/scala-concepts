...
object MapDemo {
  def main(args: Array[String]) {
    ...
    var evenNumberCalFunc = (num: Int) => {(num % 2 == 0)}: Boolean
    var (even, odd) = collection.partition(evenNumberCalFunc)
    println(even)
    println(odd)

    var squareNumbers = (num: Int) => {num * num}: Int
    ...
    var v = Vector(4, 5)
    var lv = l ++ v
    //merge the two collection take the type of left collection
    ...
    println("fold left ")
    numbers.foldLeft("")((str1: String, str2: Int) => {
      println("strings are " + str1 + " & " + str2)
      str1 + str2
    }) //reads numbers from the left,i.e index 0
    println("fold right ")
